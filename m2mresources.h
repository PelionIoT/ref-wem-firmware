...
public:
        ...
        : _cloud_client(cloud_client)
    {
        ...
        M2MObjectInstance *exec_inst =
            executable_object->create_object_instance();
        M2MResource *unreg_res = exec_inst->create_dynamic_resource(
            "1", "unregister", M2MResourceInstance::STRING, false);
        unreg_res->set_operation(M2MBase::POST_ALLOWED);
        unreg_res->set_execute_function(
            execute_callback(this, &ExecutableResource::unregister));

        M2MResource *factory_reset_res = exec_inst->create_dynamic_resource(
            "2", "factory_reset", M2MResourceInstance::STRING, false);
        factory_reset_res->set_operation(M2MBase::POST_ALLOWED);
        factory_reset_res->set_execute_function(
            execute_callback(this, &ExecutableResource::factory_reset));
    }

    M2MObject *get_object() { return executable_object; }
...
public:
    WritableResource()
    {
        ...
        M2MResource *res = obj_inst->create_dynamic_resource(
            "1", "writable_resource", M2MResourceInstance::INTEGER,
            false /* observable */);
    ...
    M2MObject *get_object() { return writable_object; }
...
public:
    ObservableResource() : counter(0)
    {
        ...
        M2MObjectInstance *obs_inst =
            observable_object->create_object_instance();
        // create resource with ID '5501', which is digital input counter
        M2MResource *obs_res = obs_inst->create_dynamic_resource(
            "1", "observable_resource", M2MResourceInstance::INTEGER,
            true /* observable */);
        // we can read this value
        obs_res->set_operation(M2MBase::GET_ALLOWED);
        // set initial value (all values in mbed Client are buffers)
        // to be able to read this data easily in the Connector console, we'll
        // use a string
    ...
    M2MObject *get_object() { return observable_object; }
